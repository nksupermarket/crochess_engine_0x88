...
import com.crochess.engine0x88.moveEval.MoveEval;
...
public class Uci {
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      if ("com/crochess/engine0x88/uci".equals(inputStr)) {
        inputUCI();
  ...
  public static int algebraToMove(String moveNotation) {
    int from =
        moveNotation.charAt(0)
            - 'a'
            + (16 * (Character.getNumericValue(moveNotation.charAt(1)) - 1));
    int to =
        moveNotation.charAt(2)
            - 'a'
            + (16 * (Character.getNumericValue(moveNotation.charAt(3)) - 1));
  ...
  public static String moveToAlgebra(int move) {
    ...
    String algebra = from.toString().toLowerCase() + to.toString().toLowerCase();
    if (promote != 0) {
