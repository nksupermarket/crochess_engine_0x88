...
public final class MoveEval {
  private MoveEval() {}

  public static void main(String[] args) {}

  static final int CHECKMATE_VAL = Integer.MAX_VALUE - 1;
  ...
  static final int TTMOVE_SORT_VAL = 60;
  ...
  private static int evaluate(int levelsSearched) {
    ...
    if (GameState.isCheckmate(Color.getOppColor(GameState.activeColor)))
      return CHECKMATE_VAL - levelsSearched;

    Color oppColor = Color.getOppColor(GameState.activeColor);
    int pawnCountDiff =
        GameState.pawnCount[GameState.activeColor.ordinal()]
            - GameState.pawnCount[oppColor.ordinal()];
    int pieceCountDiff =
        GameState.pieceCount[GameState.activeColor.ordinal()]
            - GameState.pieceCount[oppColor.ordinal()];
    int pieceBonusDiff =
        GameState.pieceBonus[GameState.activeColor.ordinal()]
  ...
  private static int quiescence(int alpha, int beta, int levelsSearched) {
    ...
    if (GameState.inCheck(GameState.activeColor)) {
      ...
      return ttVal != TranspositionTable.UNKNOWN_VAL
          ? ttVal
          : alphaBeta(1, alpha, beta, levelsSearched);
    }
    ...
    for (int i = 0; i < scores.length; i++) {
      ...
      int eval =
          ttVal != TranspositionTable.UNKNOWN_VAL
              ? ttVal
      ...
      if (eval > alpha) {
        TranspositionTable.store(
            GameState.zobristHash, 0, TT_Flag.EXACT, eval, forcingMoves.get(i));
        alpha = eval;
  ...
  private static int alphaBeta(int depth, int alpha, int beta, int levelsSearched) {
    if (depth == 0) {
      ...
      return ttVal != TranspositionTable.UNKNOWN_VAL
          ? ttVal
          : quiescence(alpha, beta, levelsSearched);
    }
    ...
    for (int i = 0; i < scores.length; i++) {
      ...
      if (!GameState.isDraw()) {
        ...
        eval =
            ttVal != TranspositionTable.UNKNOWN_VAL
                ? ttVal
                : -alphaBeta(depth - 1, -beta, -alpha, levelsSearched + 1);
      }
      ...
      if (eval > alpha) {
        TranspositionTable.store(
            GameState.zobristHash, depth, TT_Flag.EXACT, eval, legalMoves.get(i));
        alpha = eval;
    ...
    for (int i = 0; i < legalMoves.size(); i++) {
      ...
      if (!GameState.isDraw()) {
        ...
        eval =
            ttVal != TranspositionTable.UNKNOWN_VAL
                ? ttVal
                : -alphaBeta(depth - 1, Integer.MIN_VALUE + 1, -alpha, 1);
      }
      if (eval > alpha) {
        alpha = eval;
